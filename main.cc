...
void print_usage(char *exec) {
    cout << "Usage: " << exec                               					<< endl
         ...
         << "       -train       <optional: mlp training file>"  				<< endl
...
int main(int argc, char **argv) {
    ...
    if(!config_file_name.size() || !test_file_name.size() || !test_label_file_name.size() ||
	   (!weight_file_name.size() && (!train_file_name.size() || !train_label_file_name.size()))) {
        print_usage(argv[0]);
    }

    mlp_t *mlp = new mlp_t(config_file_name, test_file_name, test_label_file_name,
                           train_file_name, train_label_file_name, weight_file_name);

	if(!weight_file_name.size()){
		mlp->initialize();
		mlp->init_weights();
		mlp->read_train_file();
